...
const winnerOfTwoCells = (symbol1: PossibleWinner, symbol2: PossibleWinner) => (symbol1 === symbol2 ? symbol1 : undefined);

const or = (a: PossibleWinner, b: PossibleWinner) => a || b;

export const create = () => ({});
export const mark = (player: Player, pos: Position, board: Board) => ({...board, [posToString(pos)]: player});
export const at = (pos: Position, board: Board): PossibleWinner => board[posToString(pos)];
...
const winByRowY: WinByRowCorColXY = (y: number, board: Board) => cols.map((x) => at({x, y}, board)).reduce(winnerOfTwoCells);

const winByColX: WinByRowCorColXY = (x: number, board: Board) => rows.map((y) => at({x, y}, board)).reduce(winnerOfTwoCells);
